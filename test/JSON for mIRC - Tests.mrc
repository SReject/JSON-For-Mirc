...
alias -l jfm_test6 {
  ...
  if ($JSON(jfm_test6).Input !== %input) return $!JSON(jfm_test6).Input returned incorrectly: $v1
  ...
  if ($JSON(jfm_test6).Length !== 1) return $!JSON(jfm_test6).Length returned incorrectly: $v1
...
alias -l jfm_test7 {
  set -u0 %jfm_test.title $!JSON() walking
  
  var %input = {"null":null, "true":true, "false":false, "int":1, "dec":1.1, "negint":-1, "negdec":-1.1, "string": "jfm_test7", "array":["item0", "item1", "item2"], "object":{"key0":"item0", "key1":"item1", "key2":"item2"}}
  
  JSONOpen -d jfm_test7 %input
  
  if (JSON:jfm_test7:?* !iswm $JSON(jfm_test7, true)) return $!JSON(name, @member) should return a reference: $v2
  if ($JSONError) return $!JSON(name, @member) should create a reference

  if ($null !== $JSON(jfm_test7, null).value) return $!JSON() failed to return valid null value
  if ($JSONError) return $v1
  
  if ($true !== $JSON(jfm_test7, true).value) return $!JSON() failed to return valid boolean true value
  if ($JSONError) return $v1
  
  if ($false !== $JSON(jfm_test7, false).value) return $!JSON() failed to return valid boolean false value
  if ($JSONError) return $v1
  
  if (1 !== $JSON(jfm_test7, int).value) return $!JSON() failed to return valid numerical 1 value
  if ($JSONError) return $v1
  
  if (1.1 !== $JSON(jfm_test7, dec).value) return $!JSON() failed to return valid numerical 1.1 value
  if ($JSONError) return $v1
  
  if (-1 !== $JSON(jfm_test7, negint).value) return $!JSON() failed to return valid numerical -1 value
  if ($JSONError) return $v1

  if (-1.1 !== $JSON(jfm_test7, negdec).value) return $!JSON() failed to return valid numerical -1.1 value
  if ($JSONError) return $v1
  
  if (jfm_test7 !== $JSON(jfm_test7, string).value) return $!JSON() failed to return valid string value
  if ($JSONError) return $v1
  
  if ($JSON(jfm_test7, array).value !== $Null) return $!JSON().value should not return a value if the reference is a container
  if ($JSONError !== INVALID_TYPE) return $!JSON().value should raise an "INVALID_TYPE" error when referencing a container
  
  if (item0 !== $JSON(jfm_test7, array, 0).value) return $!json().value returning an array item's value (#1)
  if ($JSONError) return $v1
  
  if (item1 !== $JSON(jfm_test7, array, 1).value) return $!json().value returning an array item's value (#2)
  if ($JSONError) return $v1
  
  if (item2 !== $JSON(jfm_test7, array, 2).value) return $!json().value returning an array item's value (#3)
  if ($JSONError) return $v1
  
  if ($JSON(jfm_test7, object).value !== $Null) return $!JSON().value should not return a value if the reference is a container
  if ($JSONError !== INVALID_TYPE) return $!JSON().value should raise an "INVALID_TYPE" error when referencing a container
  
  if (item0 !== $JSON(jfm_test7, object, key0).value) return $!json().value returning an object item's value (#1)
  if ($JSONError) return $v1
  
  if (item1 !== $JSON(jfm_test7, object, key1).value) return $!json().value returning an object item's value (#2)
  if ($JSONError) return $v1
  
  if (item2 !== $JSON(jfm_test7, object, key2).value) return $!json().value returning an object item's value (#3)
  if ($JSONError) return $v1
  
  if ($true !== $JSON(jfm_test7, ~ TRUE).fuzzyValue) return $!JSON().FuzzyValue failed to return
  if ($JSONError) return $v1

  if ($true !== $JSON(jfm_test7, ~ 1).fuzzyValue) return $!JSON().FuzzyValue failed to return
  if ($JSONError) return $v1
}
